...
def main():
    ...
    board = Dashboard()
    editor=Editor(board, 0, 0, 11, 11, minW=3, minH=3)
    player=Player(board, 0, 12, 6, 10, minH=5)
    pie=Pie(board, 6, 0, 6, 7, minW=3, minH=4)
    radar=Radar(board, 12, 7, 3, 7, minW=2, minH=4)
    card=Card(board, 6, 7, 3, 7, minW=2, minH=4)
    ...
    editor.add_tab("Data grid", json.dumps(DataGrid.DEFAULT_ROWS, indent=2), "json")
    editor.add_tab("Radar chart", json.dumps(Radar.DEFAULT_DATA, indent=2), "json")
    editor.add_tab("Pie chart", json.dumps(Pie.DEFAULT_DATA, indent=2), "json")    
